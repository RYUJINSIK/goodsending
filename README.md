...
- `Shadcn UI` - Shadcn UI는 Tailwind CSS 기반의 UI 컴포넌트 라이브러리로, 일관성 있는 UI 개발에 유리 Tailwind UI는 Tailwind CSS 기반의 UI 킷이지만, 컴포넌트 구현이 필요하다. 개발 생산성을 고려했을 때 Shadcn UI가 더 적합할 것 같았기에 Shadcn UI 결정
...
- `Redux` - Redux는 중앙 집중식 상태 관리를 통해 예측 가능한 상태 변화를 제공하며, 단일 스토어로 일관성을 유지합니다. 불변성을 통해 상태 추적이 용이하고, 미들웨어와 개발 도구를 통해 비동기 작업 및 디버깅을 간편하게 지원합니다.
